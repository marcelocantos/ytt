...
func TestIndexPreambleRecentWindow(t *testing.T) {
	now := time.Date(2026, 8, 29, 10, 0, 0, 0, time.FixedZone("AEST", 10*3600))
	shown := []indexRow{{sortKey: "20260828"}, {sortKey: "20260822"}}
	got := indexPreamble(shown, 1876, 7, now)
	for _, want := range []string{
		"Auto-generated by `ytt build-index`. Last updated 2026-08-29T10:00+1000.",
		"## Scope",
		"last 7 days",
		"2 of 1876 ingested synopses",
...
func TestIndexPreambleEmptyTree(t *testing.T) {
	got := indexPreamble(nil, 0, 7, time.Unix(0, 0).UTC())
	if !strings.Contains(got, "## Scope") || !strings.Contains(got, "Empty — nothing ingested yet.") {
		t.Errorf("empty preamble: %q", got)
	}
}

func TestFilterRecentDropsOlderAndUndated(t *testing.T) {
	now := time.Date(2026, 8, 29, 12, 0, 0, 0, time.Local)
	rows := []indexRow{
		{sortKey: "20260829", row: "today"},
		{sortKey: "20260822", row: "week-ago"},
		{sortKey: "20260821", row: "too-old"},
		{sortKey: "20120124", row: "ancient"},
		{sortKey: "00000000", row: "undated"},
	}
	got := filterRecent(rows, now, 7)
	if len(got) != 2 || got[0].row != "today" || got[1].row != "week-ago" {
		t.Fatalf("got %+v, want today and week-ago", got)
	}
	if all := filterRecent(rows, now, 0); len(all) != len(rows) {
		t.Fatalf("days=0 should keep all, got %d", len(all))
	}
}
